...
def convolutional_model(batch_input_shape=(BATCH_NUM_TRIPLETS * 3 * NUM_FRAMES, 16, 16, 1),
                        batch_size=BATCH_NUM_TRIPLETS*3, num_frames=NUM_FRAMES):
    ...
    # the last three dimensions are: 2,2,512
    x = Reshape((2048,))(x)
    x = Lambda(lambda y: K.reshape(y, (batch_size, num_frames, 2048)), name='reshape')(x)
    x = Lambda(lambda y: K.mean(y, axis=1), name='average')(x)
    x = Dense(512, name='affine')(x)  # .shape = (BATCH_SIZE * number_of_frame_slices, 512)
